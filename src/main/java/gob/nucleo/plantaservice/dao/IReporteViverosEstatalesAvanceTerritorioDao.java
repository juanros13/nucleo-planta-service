...
@Repository
public interface IReporteViverosEstatalesAvanceTerritorioDao extends JpaRepository<AvanceViverosEstatalesTerritorioVO, Long> {
    @Query(value = "SELECT row_number() OVER (ORDER BY cad_vivero.id) AS id, " +
            ...
            "where cad_vivero_planta.fuente_abast_id = 8 " +
            "and territorios.id = ?1 " +
            "GROUP BY territorios.nombre, cad_vivero.id, cad_vivero_planta.especie_id, cad_cat_fuente_abast.nombre, cad_vivero.nombre, concat(cad_cat_especie.nombre_comun,' - ',cad_cat_especie.variedad,' (', cad_cat_especie.nombre_cientifico,') ') " +
            "order by territorios.nombre, cad_vivero.nombre " ,
            nativeQuery = true)
    List<AvanceViverosEstatalesTerritorioVO> findByAvanceViverosEstatalesTerritorio(Long idTerritorio);
    ...
    @Query(value = "SELECT row_number() OVER (ORDER BY cad_vivero.id) AS id, " +
            ...
            "order by territorios.nombre, cad_vivero.nombre " ,
            nativeQuery = true)
    List<AvanceViverosEstatalesTerritorioVO> findByAvanceViverosEstatales();
}
